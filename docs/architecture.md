...
Each user and Onshape element pair also owns one persistent Codex app-server thread. The encrypted session stores only the thread identifier and visible CAD-plan transcript; Codex rollout history stays in that user's isolated, persistent `CODEX_HOME`. Panel reloads restore the transcript, backend restarts resume the same thread, and the current Part Studio or Assembly snapshot is appended to every new turn. Conversation context therefore carries intent while Onshape remains the authority on current geometry.
...
Onshape `429` responses honor `Retry-After` for short windows. During a longer Part Studio or Assembly read throttle, planning may use the last encrypted verified snapshot and records that fact in the preview. Part Studio apply remains guarded by the snapshot's exact source microversion and `rejectMicroversionSkew: true`; Assembly apply always re-reads the live definition and rejects any changed document microversion. When Onshape accepts a Part Studio mutation, its response supplies the new feature, feature state, serialization version, and source microversion so MorAssistant can safely advance the snapshot without inventing state.
...
An active **workspace Assembly** uses a separate inspector and operation vocabulary. The trusted host reads instances, immutable source versions/microversions, configurations, mates, occurrence paths, suppression state, and absolute object-to-world transforms. Because Onshape configuration strings contain opaque option IDs, prompt-relevant component sources are resolved through the versioned element configuration schema and supplied to Sol as encoded values plus authoritative human labels; raw tokens are never treated as physical values. The validator accepts only exact complete component-source tuples and rigid proper rotations—never scaled, sheared, or mirrored matrices. It can search the FRCDesignApp catalog, resolve exact Onshape part IDs, then approval-gate insertion, placement, suppression, unsuppression, and deletion. The create response is correlated with the matching new source instance so an unrelated concurrent insertion is never moved, and a failed insert/place pair compensates by deleting only the identified new instance.
...
`npm run check` type-checks and builds every workspace, then runs unit and full-pipeline tests. The deterministic pipeline covers OAuth, Codex sign-in, Part Studio dependency/geometry inspection, Assembly detection and inspection, authoritative decoding of opaque component-configuration option IDs, FRCDesignLib source resolution, versioned component insertion and absolute placement, exact source and rigid-transform rejection, invalid-plan self-repair, approval, native mutation, microversion guards, per-operation verification, encrypted Part Studio and Assembly snapshot persistence, forced Part Studio and Assembly `429` fallbacks, fail-fast behavior, and separately approved recovery.
